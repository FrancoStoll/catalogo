        ...
        <div className="sticky top-0">
            <div className="flex justify-center items-center">
                <div className="flex flex-col gap-2 border p-4 mt-5 rounded-xl w-full">
                    {cart && cart.map((item) => (
                        <div key={item?.id} className="flex justify-between"><p className="font-bold">Nombre del producto</p>
                    ...
                    <div className="flex justify-between border-t pt-2"><p className="font-bold">Total a pagar</p>
                        <p className="font-bold text-blue-900">${cart?.reduce((total, item) => total + (item?.precio_total ?? 0), 0).toFixed(2)}</p>
                    </div>
                </div>
            </div>
